...
void *ThreadWrapper(void *dataPtr)
{
    struct DataStruct *dataStruct = (struct DataStruct *)dataPtr;
    GifSplitPalette(dataStruct->image, dataStruct->numPixels, dataStruct->firstElt, dataStruct->lastElt, dataStruct->splitElt, dataStruct->splitDist, dataStruct->treeNode, dataStruct->buildForDither, dataStruct->pal);
    return NULL;
}
...
void GifSplitPalette(uint8_t *image, int numPixels, int firstElt, int lastElt, int splitElt, int splitDist, int treeNode, bool buildForDither, GifPalette *pal)
{
    ...
    dataStruct.image = image;
    dataStruct.numPixels = subPixelsA;
    dataStruct.firstElt = firstElt;
    dataStruct.lastElt = splitElt;
    dataStruct.splitElt = splitElt - splitDist;
    dataStruct.splitDist = splitDist / 2;
    dataStruct.treeNode = treeNode * 2;
    dataStruct.buildForDither = buildForDither;
    dataStruct.pal = pal;

    pthread_t thread_id;
    pthread_create(&thread_id, NULL, ThreadWrapper, &dataStruct);
...
void GifWriteLzwImage(FILE *f, uint8_t *image, uint32_t left, uint32_t top, uint32_t width, uint32_t height, uint32_t delay, GifPalette *pPal)
{
    ...
    for (uint32_t yy = 0; yy < height; ++yy)
    {
        for (uint32_t xx = 0; xx < width; ++xx)
        {
            ...
            // GifWriteCode( f, stat, nextValue, codeSize );
